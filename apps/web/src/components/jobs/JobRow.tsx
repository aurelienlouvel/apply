...
const WORK_MODE_CONFIG: Record<WorkMode, { icon: LucideIcon; label: string; cls: string }> = {
  onsite:  { icon: Building2, label: 'On-Site', cls: 'border-zinc-200 bg-zinc-50 text-zinc-500'  },
  hybrid:  { icon: Shuffle,   label: 'Hybrid',  cls: 'border-slate-200 bg-slate-50 text-slate-500' },
  remote:  { icon: Wifi,      label: 'Remote',  cls: 'border-sky-100 bg-sky-50 text-sky-500'      },
};
...
/* ── Experience levels ──────────────────────────────────────────────────── */
const LEVEL_OPTIONS = ['Junior', 'Mid-Level', 'Senior', 'Manager', 'Founder'] as const;
...
/* ── Sectors ────────────────────────────────────────────────────────────── */
type SectorKey =
  | 'SaaS' | 'FinTech' | 'EdTech' | 'E-Commerce' | 'HealthTech'
  | 'Studio' | 'Agency' | 'IT Services' | 'LegalTech' | 'GreenTech';
...
const SECTOR_CONFIG: Record<SectorKey, { icon: LucideIcon; cls: string }> = {
  SaaS:          { icon: Cloud,        cls: 'border-blue-200 bg-blue-50 text-blue-700'          },
  FinTech:       { icon: TrendingUp,   cls: 'border-green-200 bg-green-50 text-green-700'       },
  EdTech:        { icon: BookOpen,     cls: 'border-amber-200 bg-amber-50 text-amber-700'       },
  'E-Commerce':  { icon: ShoppingCart, cls: 'border-orange-200 bg-orange-50 text-orange-700'   },
  HealthTech:    { icon: Heart,        cls: 'border-pink-200 bg-pink-50 text-pink-700'          },
  Studio:        { icon: Palette,      cls: 'border-purple-200 bg-purple-50 text-purple-700'    },
  Agency:        { icon: Globe,        cls: 'border-indigo-200 bg-indigo-50 text-indigo-700'    },
  'IT Services': { icon: Monitor,      cls: 'border-slate-200 bg-slate-50 text-slate-700'       },
  LegalTech:     { icon: Scale,        cls: 'border-rose-200 bg-rose-50 text-rose-700'          },
  GreenTech:     { icon: Leaf,         cls: 'border-emerald-200 bg-emerald-50 text-emerald-700' },
};

/* ── Fake data pools ────────────────────────────────────────────────────── */
const FAKE_SIZES     = ['2–10', '11–50', '51–200', '201–500', '500–1k', '1k+'];
const FAKE_SALARIES: (string | null)[] = [
  null, null, '35–45K', '40–55K', '45–60K', '55–75K', '70–90K', '80–100K+',
];
const FAKE_CONTRACTS = ['Full-Time', 'Full-Time', 'Full-Time', 'Part-Time', 'Freelance', 'Internship'];
const FAKE_DATES     = ['Today', 'Yesterday', '2d ago', '3d ago', '5d ago', '1w ago', '2w ago'];
...
const CONN_INITIALS  = ['AM', 'JD', 'MC', 'PL', 'SR', 'TC', 'VB', 'NF', 'KH', 'RL'];
...
/* ── Component ──────────────────────────────────────────────────────────── */
export function JobRow({ job, status, onSelect, onDecline, onApply }: JobRowProps) {
  ...
  const workModes: WorkMode[] = realMode ? [realMode] : pick(FAKE_WORK_MODES, job.id, 9);
  ...
  return (
    ...
    >
      ...
      <div className="flex w-64 shrink-0 flex-col gap-1 pr-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <p className="truncate text-sm font-semibold leading-snug text-foreground">{job.company}</p>
            <p className="truncate text-xs leading-snug text-muted-foreground">{job.title}</p>
          </div>
        </div>
        {/* Level + salary */}
        <div className="flex flex-wrap items-center gap-1.5 pl-[calc(2.5rem+0.75rem)]">
          <Badge variant="outline" className={cn('rounded-full text-[10px] font-medium', LEVEL_COLORS[level])}>
            {level}
          </Badge>
          {fakeSalary && (
            <Badge variant="outline" className="gap-0.5 rounded-full border-green-200 bg-green-50 text-[10px] font-medium text-green-700">
      ...
      <div className="w-32 shrink-0 pr-4">
        <Badge variant="outline" className={cn('gap-1 rounded-md text-[11px] font-medium', sectorCfg.cls)}>
      ...
      <div className="min-w-0 flex-[1.5] pr-4">
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-1">
            {workModes.map((mode) => {
              const { icon: Icon, label, cls } = WORK_MODE_CONFIG[mode];
              return (
                <Badge key={mode} variant="outline" className={cn('gap-1 rounded-md text-[11px] font-medium', cls)}>
      ...
      <div className="flex w-44 shrink-0 items-center justify-end gap-2">
        <Button
          variant="outline"
          size="xs"
          onClick={(e) => { e.stopPropagation(); onDecline(job.id); }}
        >
          <X className="size-3" />
          Pass
        </Button>
