    ...
    <div className="px-20 py-12">
      ...
      {interviews.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {interviews.map((iv) => {
            const companyName = iv.application.company.name;
            const jobTitle = iv.application.jobTitle;
            return (
              <li key={iv.id}>
                <Link
                  href={`/interviews/${entrySlug([companyName, jobTitle], iv.id)}`}
                ...
                >
                  <span>
                    ...
                    <span className="ml-1.5 text-muted-foreground">{jobTitle}</span>
                  </span>
